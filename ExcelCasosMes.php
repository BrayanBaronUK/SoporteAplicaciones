...
//Consulta
    ...
    while ($row = oci_fetch_array($resultado_set)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C'.$fila, '100');
        $objPHPExcel->getActiveSheet()->setCellValue('D'.$fila, '200');
        $objPHPExcel->getActiveSheet()->setCellValue('E'.$fila, $row[2]);
        $objPHPExcel->getActiveSheet()->setCellValue('F'.$fila, '80%');

        $fila++; //Sumamos 1 para pasar a la siguiente fila
    }       

    header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
	header('Content-Disposition: attachment;filename="CasosArandaMes.xls"');
